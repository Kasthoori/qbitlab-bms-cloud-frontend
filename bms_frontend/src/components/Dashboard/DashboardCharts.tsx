...
function EmptyChart({ message = "No chart data available" }: { message?: string }) {
  return (
    <BmsCard
      variant="glass"
      className="flex h-full items-center justify-center p-5 text-sm text-slate-400"
    >
      {message}
    </BmsCard>
  );
...
export function FailedHvacsBarChart({
  sites,
}: {
...
export function TemperatureBySiteChart({
  sites,
}: {
  sites: DashboardSiteCardDto[];
}) {
...
export function TenantComparisonChart({
  ...
  return (
    <GlassChartCard title="Tenant comparison" subtitle="Admin view across tenants">
      {data.length === 0 ? (
        <EmptyChart />
      ) : (
        <ResponsiveContainer width="100%" height="100%">
