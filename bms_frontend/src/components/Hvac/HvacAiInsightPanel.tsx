...
export default function HvacAiInsightPanel({
  ...
  return (
    ...
    >
      <BmsCard variant="section" className="relative overflow-hidden p-5">
        ...
        <div className="relative z-10">
          <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <BmsButton
                type="button"
                variant="ghost"
                onClick={() => void loadRuleBasedInsight()}
              ...
              <BmsButton
                type="button"
                variant="secondary"
                onClick={() => void handleOpenAiAssistance()}
                disabled={loadingOpenAi || loadingInsight || !insight}
                className="border-violet-400/30 bg-violet-400/10 text-violet-100 shadow-lg shadow-violet-500/10 hover:bg-violet-400/20"
              >
                {loadingOpenAi ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
          ...
          {error && (
            <BmsCard
              variant="glass"
              className="mb-4 border-red-400/30 bg-red-500/10 px-4 py-3 text-sm text-red-200"
            >
              {error}
            </BmsCard>
          )}
          ...
          {isInitialIdle && (
            <BmsCard variant="glass" className="p-4 text-sm text-slate-300">
          ...
          {loadingInsight && (
            <BmsCard
              variant="glass"
              className="flex items-center gap-3 p-4 text-sm text-slate-300"
            >
          ...
          {!loadingInsight && insight && (
            <div className="space-y-5">
              ...
              <BmsCard variant="glass" className="p-4">
              ...
              {openAi && (
                ...
                >
                  <BmsCard
                    variant="section"
                    className="border-violet-400/20 bg-violet-400/10 p-5 shadow-xl shadow-violet-500/10"
                  >
                    ...
                    <div className="space-y-4 text-sm leading-6 text-slate-200">
                      <BmsCard variant="glass" className="bg-black/20 p-4">
                      ...
                      <BmsCard
                        variant="glass"
                        className="border-amber-400/20 bg-amber-400/10 p-4 text-amber-100"
                      >
...
function SmallStat({ label, value }: { label: string; value: string }) {
  return (
    <BmsCard variant="glass" className="px-4 py-3">
      ...
      {items.length === 0 ? (
        ...
        <ul className="space-y-2">
          {items.map((item, index) => (
            <li
              key={`${title}-${index}`}
              className="rounded-2xl border border-white/10 bg-white/4 px-3 py-2 text-sm leading-5 text-slate-300"
            >
              {item}
            </li>
          ))}
